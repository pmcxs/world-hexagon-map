...
namespace WorldHexagonMap.Loader.HexagonParsers
{
    public class WayParser : IHexagonParser
    {
        ...
        public WayParser(IHexagonService hexagonService, HexagonDefinition hexagonDefinition)
        {
        ...
        public IEnumerable<HexagonLoaderResult> ProcessGeoData(GeoData geoData, IValueHandler valueHandler = null)
        {
            foreach (var coordinates in geoData.Points)
            {
                ...
                {
                    PointXY currentPoint = coordinates[i];
                    PointXY nextPoint = coordinates[i + 1];

                    //for each segment check intersection with the edges from the hexagons

                    IEnumerable<HexagonLocationUV> hexagons = GetHexagonsIntersectedByLine(currentPoint, nextPoint);
                    ...
                    foreach (var hexagonLocation in hexagons)
                    {
                        ...
                        for (int j = 0; j < hexagonPoints.Count - 1; j++)
                        {
                            var hexagonCurrentPoint = hexagonPoints[j];
                            var hexagonNextPoint = hexagonPoints[j + 1];

                            if (GetLineIntersection(currentPoint, nextPoint, hexagonCurrentPoint, hexagonNextPoint) != null)
                            {
